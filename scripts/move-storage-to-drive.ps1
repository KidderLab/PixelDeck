...
function Get-EnvMap {
  ...
  $map = @{}
  if (-not (Test-Path -LiteralPath $EnvPath)) { throw "Missing .env file at $EnvPath" }
...
function Get-DatabaseFilePath {
  ...
  if (-not $DatabaseUrl.StartsWith('file:', [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "Only SQLite file: DATABASE_URL values are supported by this script. Current value: $DatabaseUrl"
  }
  $relative = $DatabaseUrl.Substring(5)
  return Resolve-ProjectPath -BasePath $ProjectRootPath -Candidate $relative
}

function Ensure-Dir {
  param([string]$PathToCreate)
  if (-not (Test-Path -LiteralPath $PathToCreate)) { New-Item -ItemType Directory -Force -Path $PathToCreate | Out-Null }
}
...
function Copy-Tree {
  ...
  $code = $LASTEXITCODE
  if ($code -ge 8) { throw "Robocopy failed for $Source -> $Destination with exit code $code" }
}
...
if (-not $oldStorageRoot) { $oldStorageRoot = Join-Path $projectRootResolved 'storage' }
...
$env:PIXELDECK_IMPORTS_NEW = $targetImportsRoot
 = Join-Path ([System.IO.Path]::GetTempPath()) 'pixeldeck-move-storage.cjs'
 = @(
...
'  await prisma.("UPDATE Asset SET originalPath = replace(originalPath, ?, ?) WHERE originalPath LIKE ?", oldStorage, newStorage, likeOldStorage);',
'  await prisma.("UPDATE Asset SET thumbnailPath = replace(thumbnailPath, ?, ?) WHERE thumbnailPath LIKE ?", oldStorage, newStorage, likeOldStorage);',
'  await prisma.("UPDATE Asset SET previewPath = replace(previewPath, ?, ?) WHERE previewPath LIKE ?", oldStorage, newStorage, likeOldStorage);',
'  await prisma.("UPDATE Asset SET posterPath = replace(posterPath, ?, ?) WHERE posterPath LIKE ?", oldStorage, newStorage, likeOldStorage);',
'  await prisma.("UPDATE ExportJob SET zipPath = replace(zipPath, ?, ?) WHERE zipPath IS NOT NULL AND zipPath LIKE ?", oldStorage, newStorage, likeOldStorage);',
'  await prisma.("UPDATE ImportJob SET sourceFolder = replace(sourceFolder, ?, ?) WHERE sourceFolder LIKE ?", oldImports, newImports, likeOldImports);',
...
try {
  node 
  if ( -ne 0) { throw 'Failed to update SQLite paths after copying storage.' }
} finally {
  Pop-Location
 ...
 = Update-EnvValue -Content  -Key 'DATABASE_URL' -Value ('file:' + .Replace('\\', '/'))
 = Update-EnvValue -Content  -Key 'PIXELDECK_STORAGE_ROOT' -Value (.Replace('\\', '/'))
 = Update-EnvValue -Content  -Key 'PIXELDECK_IMPORT_ROOT' -Value (.Replace('\\', '/'))
 = Update-EnvValue -Content  -Key 'PIXELDECK_ORIGINALS_ROOT' -Value (.Replace('\\', '/'))
 = Update-EnvValue -Content  -Key 'PIXELDECK_THUMBS_ROOT' -Value (.Replace('\\', '/'))
 = Update-EnvValue -Content  -Key 'PIXELDECK_PREVIEWS_ROOT' -Value (.Replace('\\', '/'))
 = Update-EnvValue -Content  -Key 'PIXELDECK_POSTERS_ROOT' -Value (.Replace('\\', '/'))
 = Update-EnvValue -Content  -Key 'PIXELDECK_ZIPS_ROOT' -Value (.Replace('\\', '/'))
 = Update-EnvValue -Content  -Key 'PIXELDECK_LOG_ROOT' -Value (.Replace('\\', '/'))
[System.IO.File]::WriteAllText(, , [System.Text.UTF8Encoding]::new(False))
if ($RemoveOldStorage) {
