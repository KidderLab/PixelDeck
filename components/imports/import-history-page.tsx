...
export function ImportHistoryPage() {
  ...
  const load = useCallback(async () => {
    ...
    setSelectedJobId((current) => current ?? payload.data[0]?.id ?? null);
  }, []);
        ...
        <div className="mt-4 space-y-3">
          {jobs.map((job) => {
            ...
            return (
              <button
                key={job.id}
                className={`w-full rounded-2xl border p-4 text-left transition ${isActive ? "border-cyan-400/40 bg-cyan-500/10" : "border-white/10 bg-white/[0.03] hover:bg-white/[0.05]"}`}
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="space-y-3">
                {(selectedJob.duplicateEvents ?? []).length === 0 ? (
                  <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4 text-sm text-muted-foreground">No duplicate matches recorded for this import job yet.</div>
                ) : (
                  selectedJob.duplicateEvents.map((event) => (
